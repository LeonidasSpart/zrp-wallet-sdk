...
import (
	...
	"github.com/LeonidasSpart/zrp-wallet-sdk/crypto/btcd/btcec"
	"github.com/LeonidasSpart/zrp-wallet-sdk/crypto/btcd/chaincfg"
	"github.com/LeonidasSpart/zrp-wallet-sdk/crypto/btcd/wire"
)
...
// specified address.
func PayToAddrScript(addr btcec.Address) ([]byte, error) {
	const nilAddrErrStr = "unable to generate payment script for nil address"

	switch addr := addr.(type) {
	case *btcec.AddressPubKeyHash:
		if addr == nil {
	...
	case *btcec.AddressWitnessPubKeyHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
		return payToWitnessPubKeyHashScript(addr.ScriptAddress())
	case *btcec.AddressWitnessScriptHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
...
func MultiSigScript(pubkeys []*btcec.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []btcec.Address, int, error) {
	var addrs []btcec.Address
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := btcec.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case WitnessV0PubKeyHashTy:
		...
		// Skip the pubkey hash if it's invalid for some reason.
		requiredSigs = 1
		addr, err := btcec.NewAddressWitnessPubKeyHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := btcec.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := btcec.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case WitnessV0ScriptHashTy:
		...
		// Skip the script hash if it's invalid for some reason.
		requiredSigs = 1
		addr, err := btcec.NewAddressWitnessScriptHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]btcec.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := btcec.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
