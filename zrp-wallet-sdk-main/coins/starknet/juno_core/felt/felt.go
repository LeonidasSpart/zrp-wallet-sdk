...
import (
	...
	"github.com/LeonidasSpart/zrp-wallet-sdk/coins/starknet/juno_core/fp"
)
...
func HexToFelt(hex string) *Felt {
	f, _ := new(Felt).SetString(hex)
	return f
}

func FeltToBigInt(f *Felt) *big.Int {
	tmp := f.Bytes()
